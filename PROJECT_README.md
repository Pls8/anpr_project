...
| v7          | EfficientNet-B0 | 79.53%     | 100    | 38 chars    | Continue from v5 aug               |

---

### EfficientNet-B1 Experiments

We tested EfficientNet-B1 (larger model) to try to improve accuracy:

| Version | Approach | Dataset | Result |
|---------|----------|---------|--------|
| B1 v1 | Train from scratch | Original (~10k) | 78.89% (43 epochs) |
| B1 v2 | Train from scratch, batch 32 | Original (~10k) | 79.53% (50 epochs) |
| B1 v2 | Continue training | Original (~10k) | 79.53% (plateaued) |
| B1 v3 | Train with v1 dataset | Original + v1 (~12k) | 78.80% (noisy labels hurt) |
| B0 v7 | Continue from v5_aug | Original (~10k) | 79.53% (100 epochs) |
| B0 v8 | More augmentation | Original (~10k) | 75.79% (worse!) |

**Conclusion:** 
- EfficientNet-B1 did NOT outperform EfficientNet-B0
- The augmented model (v5_aug) provides best generalization
- More aggressive augmentation hurt performance
- Original dataset size (~10k images) limits further improvement
- **To reach 80%+, more/better training data is needed**

#### B1 Training Parameters (for reference):
- **Image Size:** 240x240 (vs 224x224 for B0)
- **Batch Size:** 32
- **Epochs:** 50-100
